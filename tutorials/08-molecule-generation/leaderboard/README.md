...
leaderboard challenge in `molecule-generation.ipynb`. It diverges from the pattern used by
`tutorials/06-pretraining_finetuning/leaderboard/` in one structural way: **students never self-report a
...
- `apps_script.gs` — Google Apps Script Web App: dispatches on the `action` field, appending to
  ...
  `pretraining_finetuning`'s endpoint — no auth distinction between a student's submission and the
...
There's no held-out dataset here (unlike `pretraining_finetuning`) — every submission is scored fresh
