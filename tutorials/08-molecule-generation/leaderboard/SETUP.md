...
grading step can't live purely in the Sheet like `pretraining_finetuning`'s does).
...
- In `molecule-generation.ipynb`'s challenge cell, set `LEADERBOARD_ENDPOINT_URL` to the Web App URL
  from step 2.4.
...
- In `molecule-generation.html`, set `LEADERBOARD_SHEET_ID` and `LEADERBOARD_GID` (in the `<script>`
...
- **The tutorial page shows "leaderboard not configured yet."** `LEADERBOARD_SHEET_ID` /
  `LEADERBOARD_GID` in `molecule-generation.html` still have placeholder values, or the `Leaderboard`
