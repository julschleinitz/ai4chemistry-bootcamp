...
for `tutorials/neural-networks-mlp/leaderboard/` — a Google Sheet plus an Apps Script Web App, no
...
the repo alongside the notebook, at `tutorials/few-shot-learning/leaderboard/official_test_set.csv`
...
- In `few-shot-learning.ipynb`, set `LEADERBOARD_ENDPOINT_URL` (in the challenge section) to the
  Web App URL from step 3.
- In `few-shot-learning.html`, set `LEADERBOARD_SHEET_ID` and `LEADERBOARD_GID` (in the `<script>`
...
- **The tutorial page shows "leaderboard not configured yet."** `LEADERBOARD_SHEET_ID` /
  `LEADERBOARD_GID` in `few-shot-learning.html` still have their placeholder values — fill them in
